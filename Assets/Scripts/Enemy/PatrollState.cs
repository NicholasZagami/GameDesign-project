...
public class PatrollState : StateMachineBehaviour
{
    ...
    [SerializeField] float waitAtPoint = 0.2f;  // piccolo delay per stabilit�
    ...
    public override void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        {
            Debug.LogWarning($"{animator.name}: 'PatrolPoints' � vuoto.");
            animator.SetBool("isPatrolling", false);
            return;
        }

        // Opzionale: parti dal punto pi� vicino
        if (startFromNearest)
        {
    ...
    void MoveToCurrent()
    {
        // Se il punto non � su NavMesh, prova a "snapparlo" alla NavMesh
    ...
    public override void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        // Avanza in ordine, non pi� random
        if (!agent.pathPending)
        {
            ...
            if (agent.remainingDistance <= agent.stoppingDistance + 0.05f)
            {
                ...
                // aspetta un attimo per stabilit�
        ...
        // Passaggio a chase se il player � vicino
        if (player != null)
        {
